...
class MPCControl_roll(MPCControl_base):
    ...
    def get_u(
        ...
        assert self.ocp.status == cp.OPTIMAL, \
            f"The nominal roll mpc solver returned status: {self.ocp.status}"
    ...
    def generate_constraint(xs, us, B):
        ...
        wz_min_phys, wz_max_phys = -150.0, 150.0
        roll_min_phys, roll_max_phys = -np.deg2rad(80), np.deg2rad(80)
        lower_X = np.array([
            wz_min_phys - xs[0],
            roll_min_phys - xs[1]
        ])
        ...
        Pdiff_min_phys, Pdiff_max_phys = -20.0, 20.0
        Pdiff_min = Pdiff_min_phys - us[0]
        Pdiff_max = Pdiff_max_phys - us[0]
        A_U = np.vstack((np.eye(1), -np.eye(1)))
        b_U = np.array([Pdiff_max, -Pdiff_min])
